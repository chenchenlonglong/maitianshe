...
class Query extends Component implements QueryInterface
{
    ...
    public $distinct;
    /**
     * @var array the table(s) to be selected from. For example, `['member', 'post']`.
     ...
     * ```php
     * [
     *     ['INNER JOIN', 'member', 'member.id = author_id'],
     ...
     *
     * ```php
     * $query = (new Query)->from('member');
     * foreach ($query->batch() as $rows) {
     *     // $rows is an array of 10 or fewer rows from member table
     * }
     * ```
     *
     ...
     * Columns can be prefixed with table names (e.g. "member.id") and/or contain column aliases (e.g. "member.id AS user_id").
     ...
     * @param string|array $tables the table(s) to be selected from. This can be either a string (e.g. `'member'`)
     * or an array (e.g. `['member', 'profile']`) specifying one or several table names.
     * Table names can contain schema prefixes (e.g. `'public.member'`) and/or table aliases (e.g. `'member u'`).
     ...
     *
     * ```php
     * // SELECT * FROM  `member` `u`, `profile`;
     * $query = (new \yii\db\Query)->from(['u' => 'member', 'profile']);
     *
     * // SELECT * FROM (SELECT * FROM `member` WHERE `active` = 1) `activeusers`;
     * $subquery = (new \yii\db\Query)->from('member')->where(['active' => true])
     * $query = (new \yii\db\Query)->from(['activeusers' => $subquery]);
     *
     * // subquery can also be a string with plain SQL wrapped in parenthesis
     * // SELECT * FROM (SELECT * FROM `member` WHERE `active` = 1) `activeusers`;
     * $subquery = "(SELECT * FROM `member` WHERE `active` = 1)";
     * $query = (new \yii\db\Query)->from(['activeusers' => $subquery]);
     * ```
     *
     * @return $this the query object itself
     ...
     * The table name can contain a schema prefix (e.g. 'public.member') and/or table alias (e.g. 'member u').
