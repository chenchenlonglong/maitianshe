...
interface ManagerInterface
{
     ...
     * @param string|integer $userId the member ID. This should be either an integer or a string representing
     * the unique identifier of a member. See [[\yii\web\User::id]].
     ...
     * @return boolean whether the member has the specified permission.
    ...
    /**
     * Returns the roles that are assigned to the member via [[assign()]].
     * Note that child roles that are not assigned directly to the member will not be returned.
     * @param string|integer $userId the member ID (see [[\yii\web\User::id]])
     * @return Role[] all roles directly assigned to the member. The array is indexed by the role names.
     */
    ...
    /**
     * Returns all permissions that the member has.
     * @param string|integer $userId the member ID (see [[\yii\web\User::id]])
     * @return Permission[] all permissions that the member has. The array is indexed by the permission names.
     */
     ...
     * @throws \Exception if the role has already been assigned to the member
     */
    ...
    /**
     * Revokes all roles from a member.
     * @param mixed $userId the member ID (see [[\yii\web\User::id]])
     * @return boolean whether the revoking is successful
     */
    public function revokeAll($userId);

    /**
     * Returns the assignment information regarding a role and a member.
     * @param string $roleName the role name
     * @param string|integer $userId the member ID (see [[\yii\web\User::id]])
     * @return null|Assignment the assignment information. Null is returned if
     * the role is not assigned to the member.
     */
    public function getAssignment($roleName, $userId);

    /**
     * Returns all role assignment information for the specified member.
     * @param string|integer $userId the member ID (see [[\yii\web\User::id]])
     * @return Assignment[] the assignments indexed by role names. An empty array will be
     * returned if there is no role assigned to the member.
     */
    public function getAssignments($userId);

    /**
     * Returns all member IDs assigned to the role specified.
     * @param string $roleName
     * @return array array of member ID strings
     * @since 2.0.7
     */
