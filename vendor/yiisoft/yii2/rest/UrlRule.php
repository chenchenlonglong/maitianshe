 ...
 *
 * - `'PUT,PATCH users/<id>' => 'member/update'`: update a member
 * - `'DELETE users/<id>' => 'member/delete'`: delete a member
 * - `'GET,HEAD users/<id>' => 'member/view'`: return the details/overview/options of a member
 * - `'POST users' => 'member/create'`: create a new member
 * - `'GET,HEAD users' => 'member/index'`: return a list/overview/options of users
 * - `'users/<id>' => 'member/options'`: process all unhandled verbs of a member
 * - `'users' => 'member/options'`: process all unhandled verbs of member collection
 *
 ...
 * For example, the following code will disable the `delete` rule and generate rules for both `member` and `post` controllers:
 *
 * ```php
 * [
 *     'class' => 'yii\rest\UrlRule',
 *     'controller' => ['member', 'post'],
 *     'except' => ['delete'],
 * ]
 * ```
 *
 ...
 * The controller ID used in the pattern will be automatically pluralized (e.g. `member` becomes `users`
...
class UrlRule extends CompositeUrlRule
{
    ...
    public $suffix;
    /**
     * @var string|array the controller ID (e.g. `member`, `post-comment`) that the rules in this composite rule
     * are dealing with. It should be prefixed with the module ID if the controller is within a module (e.g. `user/member`).
     *
     ...
     * the actual controller ID. For example, `['u' => 'member']`.
     *
     * You may also pass multiple controller IDs as an array. If this is the case, this composite rule will
     * generate applicable URL rules for EVERY specified controller. For example, `['member', 'post']`.
     */
    public $controller;
    /**
     ...
     * If true, a controller ID will appear in plural form in URLs. For example, `member` controller
