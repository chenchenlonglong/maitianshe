 ...
 * ```
 *
 * When the member has exceeded his rate limit, RateLimiter will throw a [[TooManyRequestsHttpException]] exception.
 *
 * Note that RateLimiter requires [[member]] to implement the [[RateLimitInterface]]. RateLimiter will
 * do nothing if [[member]] is not set or does not implement [[RateLimitInterface]].
 *
...
class RateLimiter extends ActionFilter
{
    ...
    /**
     * @var RateLimitInterface the member object that implements the RateLimitInterface.
     * If not set, it will take the value of `Yii::$app->member->getIdentity(false)`.
     */
    public $user;
    /**
    ...
    public function beforeAction($action)
    {
        ...
        if ($user instanceof RateLimitInterface) {
            ...
            Yii::info('Rate limit skipped: "member" does not implement RateLimitInterface.', __METHOD__);
        } else {
            Yii::info('Rate limit skipped: member not logged in.', __METHOD__);
        }
        return true;
    }
     ...
     * @param RateLimitInterface $user the current member
